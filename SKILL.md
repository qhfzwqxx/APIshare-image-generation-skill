...
For every image edit, plan the prompt generically instead of adding special-case color or object rules. User-requested changes have the highest priority and must be visible. Preserve only the elements the user did not ask to change, and prevent returning the original image unchanged. The script applies this prompt planning automatically for `--image` and `--image-url`.
